...
let agenda = {
  ...
  isLastVisible: () => {
    let result = 0;
    let parentHeight = agenda.sliderWrapper.parentNode.clientHeight;
    for (
      let i = agenda.breakPoints[agenda.breakPoints.length - 1];
      i < agenda.sliderWrapper.children.length;
      i++
    ) {
      let element = agenda.sliderWrapper.children[i];
      result += element.clientHeight;
    }

    return result <= parentHeight;
  },
  getTdistanceToTop: (element) => {
    var yPosition = 0;
    while (element) {
      yPosition += element.offsetTop - element.scrollTop + element.clientTop;
      element = element.offsetParent;
  ...
  findNextElement: () => {
    let parentHeight = agenda.sliderWrapper.parentNode.clientHeight;
    let index = agenda.breakPoints[agenda.breakPoints.length - 1];
    let count = 0;
    while (count <= parentHeight) {
      count += agenda.sliderWrapper.children[index].clientHeight;
      index += 1;
    }
    agenda.breakPoints.push(index - 1);
    return agenda.sliderWrapper.children[index - 1];
  },
  next: () => {
    if (agenda.allowSlide) {
      agenda.allowSlide = false;
      let elementToActivate = agenda.findNextElement();

      let distance =
        agenda.getTdistanceToTop(agenda.sliderWrapper) -
        agenda.getTdistanceToTop(elementToActivate);
      ...
      if (agenda.isLastVisible()) {
        agenda.nextButton.style.opacity = "0";
        agenda.nextButton.style.pointerEvents = "none";
      }
    }
  },
  prev: () => {
    if (agenda.allowSlide) {
      agenda.allowSlide = false;
      agenda.breakPoints.pop();
      let elementToActivate =
        agenda.sliderWrapper.children[
          agenda.breakPoints[agenda.breakPoints.length - 1]
        ];
      let distance =
        agenda.getTdistanceToTop(agenda.sliderWrapper) -
        agenda.getTdistanceToTop(elementToActivate);
      ...
      if (agenda.breakPoints.length === 1) {
